...
class ConsoleWriter(StatsWriter):
    def __init__(self):
        ...
        self.rank = get_rank()
        # Escalated envs only ever go from 0 -> 1 (see carAgent.cs), so summing each window's
        # Custom/EscalatedEnvironments reports onto this running total gives the count of distinct
        # environments that have escalated so far, for the whole run. Custom/NumEnvs is reported
    ...
    def write_stats(
        ...
        if "Environment/Cumulative Reward" in values:
            ...
            log_info.append(f"Episodes: {stats_summary.num}")
            ...
            if "Environment/Episode Length" in values:
                ep_len = values["Environment/Episode Length"]
                log_info.append(f"Mean Episode Length: {ep_len.mean:0.3f}")
                log_info.append(f"Std of Episode Length: {ep_len.std:0.3f}")
            if "Custom/GoalReached" in values:
                log_info.append(f"Goals: {int(values['Custom/GoalReached'].sum)}")
            if "Custom/Terminated" in values:
